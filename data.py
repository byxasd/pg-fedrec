...
class SampleGenerator(object):
    ...
    def _split_loo(self, ratings):
        """选择倒数第二个用于验证，最后一个用于训练"""
        ratings['rank_latest'] = ratings.groupby(['userId'])['timestamp'].rank(method='first', ascending=False)  #按时间顺序进行排序
        #选择test，val，train所用到的数据
    ...
    def _sample_negative(self, ratings):
        ...
        #得到每个user对应的所有的负样本以及选取的用于训练的负样本
        ...
        for userId, user_train_ratings in grouped_train_ratings:
            ...
            for row in user_train_ratings.itertuples():
                ...
                for i in range(num_negatives):
                    ...
                    user_rating.append(float(0))             #负采样的得分设置为0
