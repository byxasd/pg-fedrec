...
def aggAvg(round_user_params):
    # 收集所有用户的物品嵌入矩阵
    item_embeddings = [params['embedding_item.weight'] for params in round_user_params.values()]

    # 计算平均物品嵌入矩阵
    avg_embedding = sum(item_embeddings) / len(item_embeddings)

    # 为每个用户分配相同的平均嵌入
    item_embedding_dict = {}
    for user in round_user_params.keys():
        item_embedding_dict[user] = avg_embedding

    item_embedding_dict['global'] = avg_embedding
    return item_embedding_dict
