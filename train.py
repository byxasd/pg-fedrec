...
parser.add_argument('--edge_servers_num', type=int, default=20)
...
parser.add_argument('--neighborhood_threshold', type=float, default=0.5)
...
elif config['dataset'] == 'HetRec2011':
...
rating = rating[['userId', 'itemId', 'rating', 'timestamp']]    #rating包含了所有的user的交互过的item的rating打分
print(rating)
...
validate_data = sample_generator.validate_data   #验证时的数据  #[torch.LongTensor(val_users), torch.LongTensor(val_items), torch.LongTensor(negative_users),torch.LongTensor(negative_items)]
test_data = sample_generator.test_data       #测试时的数据

'''              计算得到包含了user的交互数据的字典             '''
user_item_interactions = rating.groupby('userId')['itemId'].apply(list).to_dict()
